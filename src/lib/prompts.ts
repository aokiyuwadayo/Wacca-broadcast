...
4. platforms … 各プラットフォーム向けの完成文面（line / teams / discord / slack）
...
## slack（Slack向け）
- Slack の mrkdwn 記法に従う：太字は *アスタリスク1個*。# や ## の見出し記法・**二重アスタリスク**は使わない（そのまま表示されてしまう）。
- 見出しは「*太字*＋絵文字」で表現し、箇条書きは「・」を使う。
- 中くらいの長さ。親しみがありつつ整ったトーン（LINEよりやや整然、Teamsよりやわらかい）。
...
export function buildUserText(body: ComposeBody): string {
  ...
  const ALL_PF = ["line", "teams", "discord", "slack"];
  const pfList =
