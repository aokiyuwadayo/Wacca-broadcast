...
```bash
git clone https://github.com/aokiyuwadayo/yuwa-broadcast.git   # or git pull
cd yuwa-broadcast && npm install
...
`SYSTEM` 冒頭は今も『あなたは「YUWA Broadcast」の…』。プロダクト名は **Wacca / Wacca Cast**
に改名済み（UI 反映済み）だが、ユーザー判断により**名前変更は後回し**。
将来まとめて変える際の対象は次の3層（混同注意）:

1. **ブランド自己名称**（ここで変える対象）: `prompts.ts` SYSTEM・README・CLAUDE.md・design-doc タイトル → Wacca Cast
2. **姉妹アプリ "YUWA"**（変えない）: サークル管理本体。`github.com/aokiyuwadayo/yuwa`、「YUWA に吸収」等はそのまま
3. **リポ/インフラ名 `yuwa-broadcast`**（別ステップ・要アカウント操作）: GitHub リポ名・`package.json` name・Vercel URL
