...
> **作成日**: 2026-06-08
> **状態**: B1/B2 は `src/lib/prompts.ts` に適用済み。実出力の最終判断は個人PC（鍵あり）で行う。
> **補足**: 会社PCには Anthropic 鍵を置かない方針のため、このリポジトリ上ではプロンプト構成の
> 単体テストと build 検証までを行う。
...
### 適用した差分（`src/lib/prompts.ts` の SYSTEM 内、datetime の行を置換）
```diff
